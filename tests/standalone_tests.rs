...
mod credential_detection_tests {
    ...
    fn test_aws_credential_patterns() {
        ...
        for (input, expected) in test_cases {
            ...
            assert_eq!(
                ...
                "Extracted key doesn't match expected"
            );
        }
    }
    ...
    fn test_github_token_patterns() {
        ...
        for (input, should_match) in test_cases {
            ...
            assert_eq!(
                found, should_match,
                "GitHub token pattern test failed for: {}",
                input
            );
        }
    }
    ...
    fn test_entropy_analysis() {
        ...
        for (input, should_be_high_entropy) in test_cases {
            ...
            assert_eq!(
                is_high_entropy, should_be_high_entropy,
                "Entropy test failed for '{}'. Entropy: {:.2}",
                input, entropy
            );
        }
    }
    ...
    fn test_jwt_token_detection() {
        ...
        assert!(
            jwt_pattern.is_match(valid_jwt),
            "Valid JWT should match pattern"
        );
        assert!(
            !jwt_pattern.is_match(invalid_jwt),
            "Invalid JWT should not match pattern"
        );
        ...
        assert_ne!(
            invalid_parts.len(),
            3,
            "Invalid JWT should not have exactly 3 parts"
        );
    }
    ...
    fn test_api_key_patterns() {
        ...
        let test_patterns: Vec<(&str, &str)> = vec![
            // Stripe API keys
            (
                "sk_test_ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890abcd",
                r"sk_test_[a-zA-Z0-9]{40}",
            ),
            (
                "pk_live_ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890abcd",
                r"pk_live_[a-zA-Z0-9]{40}",
            ),
            // Slack tokens
            (
                slack_token.as_str(),
            ...
            // SendGrid API keys
            (
                "SG.ABCDEFGHIJKLMNOPQRSTUV.1234567890123456789012345678901234567890ABC",
        ...
        for (test_key, pattern_str) in test_patterns {
            ...
            assert!(
                pattern.is_match(test_key),
                "API key '{}' should match pattern '{}'",
                test_key,
                pattern_str
            );
        }
    ...
    fn test_false_positive_filtering() {
        ...
        for (credential, should_be_filtered) in test_credentials {
            let lowered = credential.to_ascii_lowercase();
            let keywords = ["test", "demo", "example", "sample", "placeholder"];
            let contains_keyword = keywords.iter().any(|kw| lowered.contains(kw));
            let explicit_live_key = lowered.starts_with("sk_live_");

            let is_test_credential = contains_keyword && !explicit_live_key;

            assert_eq!(
                is_test_credential, should_be_filtered,
                "False positive filtering failed for '{}'",
                credential
            );
        }
    }
    ...
    fn test_credit_card_luhn_validation() {
        ...
        for (card_number, expected_valid) in test_cases {
            ...
            assert_eq!(
                ...
                "Luhn validation failed for card number '{}'",
                card_number
            );
        }
    }
    ...
    fn test_context_based_confidence() {
        ...
        for (text, base_conf, should_be_high) in test_cases {
            ...
            assert_eq!(
                is_high_confidence, should_be_high,
...
mod performance_tests {
    ...
    fn test_pattern_matching_performance() {
        ...
        let slack_token = format!(
            "xoxb-{}-{}-{}",
            "1234567890123", "9876543210987", "abcdefghijklmnopqrstuvwx"
        );
        let test_content = format!(
            "AWS_ACCESS_KEY_ID=AKIAIOSFODNN7EXAMPLE
        GITHUB_TOKEN=ghp_ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890
        STRIPE_KEY=sk_test_ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890abcd
        SLACK_TOKEN={}
    ...
    fn test_entropy_calculation_performance() {
        ...
        assert!(
            duration.as_secs() < 2,
            "Entropy calculation took too long: {:?}",
            duration
        );
    }
}
