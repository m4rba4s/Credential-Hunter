...
use super::correlators::{CorrelatedEvent, EventCorrelator};
use super::events::{EventSeverity, EventType, SiemEvent};
use super::formatters::{EventFormatter, FormatType};
use super::health::{HealthMonitor, HealthStatus};
use super::logging::{LogEntry, LogLevel, SecureLogger};
use super::masking::{DataMasker, MaskingPolicy};
use super::transports::{SiemTransport, TransportType};
use super::{AuthConfig, AuthType, SiemError, SiemIntegrationConfig, SiemStats};
...
impl SiemIntegration {
    ...
    pub async fn new(config: SiemConfig) -> Result<Self> {
        ...
        let secure_logger = Arc::new(
            SecureLogger::new(&integration_config.logging_config)
                .await
        ...
        let data_masker = Arc::new(
            DataMasker::new(&integration_config.masking_config)
                .await
        ...
        let event_correlator = Arc::new(
            EventCorrelator::new(&integration_config.correlation_config)
                .await
        ...
        let event_formatter = Arc::new(
            EventFormatter::new(&config.platform)
                .await
        ...
        let health_monitor = Arc::new(
            HealthMonitor::new(&integration_config.health_config)
                .await
        ...
        let worker_semaphore = Arc::new(Semaphore::new(
            integration_config.performance_config.worker_threads,
        ));
    ...
    pub async fn send_detections(&self, detections: &[DetectionResult]) -> Result<()> {
        ...
        for detection in detections {
            ...
            let siem_event = self
                .create_siem_event_from_detection(&masked_detection)
                .await?;
        ...
        info!(
            "✅ Successfully sent {} detections to SIEM",
            detections.len()
        );
        Ok(())
    }
    ...
    async fn flush_event_batch(&self, events: Vec<SiemEvent>) -> Result<()> {
            ...
            match &send_result {
                Ok(_) => {
                    ...
                    stats.avg_latency_ms =
                        (stats.avg_latency_ms + send_duration.as_millis() as u64) / 2;
                }
                Err(_) => {
        ...
        // Log the operation
        self.log_batch_operation(&events, &send_result, send_duration)
            .await?;
    ...
    async fn create_siem_event_from_detection(
        &self,
        ...
        custom_fields.insert(
            ...
            detection.credential_type.to_string(),
        );
        ...
        custom_fields.insert(
            ...
            detection.location.source_type.clone(),
        );
    ...
    async fn log_batch_operation(
        ...
        let log_entry = LogEntry {
            ...
            message: format!(
                "Batch operation: {} events, duration: {}ms",
                events.len(),
                duration.as_millis()
            ),
            metadata: {
    ...
    async fn batch_flush_timer(self: &Arc<Self>) {
        let mut interval = tokio::time::interval(Duration::from_millis(
            self.config.performance_config.batch_timeout_ms,
        ));
    ...
    async fn health_monitoring_loop(self: &Arc<Self>) {
        let mut interval = tokio::time::interval(Duration::from_secs(
            self.config.health_config.check_interval_sec,
        ));
