...
impl EntropyAnalyzer {
    ...
    pub fn with_simd(
        ...
        let charset_analyzers = vec![
            // Hexadecimal character set (place first to avoid misclassifying pure-hex as base64)
            CharsetAnalyzer {
                name: "hex".to_string(),
    ...
    pub async fn analyze_text(&self, text: &str) -> Vec<EntropyResult> {
        ...
        debug!(
            "Entropy analysis found {} high-entropy strings",
            results.len()
        );
        results
    }
    ...
    unsafe fn extract_candidates_avx2(&self, bytes: &[u8], candidates: &mut Vec<CandidateString>) {
        ...
        while i + 32 <= bytes.len() {
            ...
            let delimiters = _mm256_or_si256(
                ...
                _mm256_or_si256(_mm256_or_si256(is_newline, is_quote), is_equals),
            );
    ...
    fn analyze_string(&self, candidate: &CandidateString) -> Option<EntropyResult> {
        ...
        let potential_type =
            self.infer_credential_type(&candidate.value, &char_distribution, &charset);
    ...
    /// Adjust confidence based on character distribution
    fn adjust_confidence_for_distribution(
        &self,
        mut confidence: f64,
        ...
        if distribution.unique_chars as f64
            / (distribution.uppercase_ratio
                + distribution.lowercase_ratio
                + distribution.digit_ratio
                + distribution.special_ratio)
            > 0.7
        {
            confidence *= 1.2;
        }
...
mod tests {
    ...
    fn test_shannon_entropy_calculation() {
        ...
        assert!(
            diff < 0.01,
            "SIMD and scalar entropy should match: SIMD={:.6}, Scalar={:.6}, Diff={:.6}",
            simd_entropy,
            scalar_entropy,
            diff
        );
    ...
    async fn test_high_entropy_detection() {
        ...
        let high_entropy_values: Vec<&str> = results.iter().map(|r| r.value.as_str()).collect();
    ...
    async fn test_simd_performance_and_accuracy() {
        ...
        for (simd_result, scalar_result) in simd_results.iter().zip(scalar_results.iter()) {
            assert_eq!(
                simd_result.value, scalar_result.value,
            ...
            assert!(
                ...
                simd_result.value,
                simd_result.entropy,
                scalar_result.entropy
            );
        }

        // Both should detect high-entropy credentials
        assert!(
            !simd_results.is_empty(),
            "Should detect high-entropy strings"
        );

        let high_entropy_values: Vec<&str> =
            simd_results.iter().map(|r| r.value.as_str()).collect();

        // Should find the API key and password
        assert!(
            high_entropy_values.iter().any(|&v| v.contains("sk_demo_")),
            "Should detect API key"
        );
        assert!(
            high_entropy_values
                .iter()
                .any(|&v| v.contains("P@ssw0rd123")),
    ...
    fn test_simd_availability() {
        ...
        #[cfg(target_arch = "x86_64")]
        {
            ...
            assert_eq!(
                forced_simd.simd_enabled, has_simd,
                "Should enable SIMD only when available"
            );
            assert!(
                !forced_no_simd.simd_enabled,
                "Should disable SIMD when forced off"
            );
        }
