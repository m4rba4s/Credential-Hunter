...
impl MLClassifier {
    ...
    pub async fn classify_text(&self, text: &str) -> Result<Vec<MLResult>> {
        ...
        debug!(
            "ML classification found {} potential credentials",
            results.len()
        );
        Ok(results)
    }
    ...
    fn extract_character_features(
        &self,
        text: &str,
    ) -> (Vec<String>, Vec<f64>, HashMap<String, String>) {
    ...
    fn extract_ngram_features(
        ...
        // Extract n-grams
        let mut ngram_counts: std::collections::HashMap<String, usize> =
            std::collections::HashMap::new();
    ...
    fn extract_statistical_features(
        ...
        let ascii_printable_count = text
            .chars()
    ...
    fn extract_structural_features(
        ...
        // Pattern detection
        let has_base64_pattern = text
            .chars()
            .all(|c| c.is_alphanumeric() || c == '+' || c == '/' || c == '=')
            && text.len() % 4 == 0;

        let has_hex_pattern = text.chars().all(|c| c.is_ascii_hexdigit()) && text.len() >= 8;
    ...
    fn extract_domain_features(
        ...
        values.push(if length_matches != "other_length" {
            1.0
        } else {
            0.0
        });
    ...
    fn ensemble_vote(&self, predictions: &[ModelPrediction]) -> (String, f64) {
        let mut class_votes: std::collections::HashMap<String, f64> =
            std::collections::HashMap::new();
        ...
        for prediction in predictions {
            ...
            *class_votes
                .entry(prediction.predicted_class.clone())
    ...
    async fn update_stats(&self, results: &[MLResult], processing_time: f64) {
        ...
        let per_result_avg = if results.is_empty() {
            processing_time
        } else {
            results
                .iter()
        ...
        // Update confidence distribution and feature telemetry
        for result in results {
            ...
            *stats
            ...
            for (name, value) in result
                ...
                .names
                .iter()
                .zip(result.features.values.iter())
            {
                *stats.feature_importance.entry(name.clone()).or_insert(0.0) += value.abs();
            }

            for (meta_key, meta_value) in &result.features.metadata {
                let key = format!("meta:{}={}", meta_key, meta_value);
                *stats.feature_importance.entry(key).or_insert(0.0) += 1.0;
            }

            let credential_key = format!("{:?}", result.credential_type);
            let performance_entry = stats
                .model_performance
                .entry(credential_key)
                .or_insert_with(ModelPerformance::default);

            performance_entry.predictions_made += 1;
            let count = performance_entry.predictions_made as f64;
            performance_entry.avg_confidence = if count <= 1.0 {
                result.confidence
            } else {
                ((performance_entry.avg_confidence * (count - 1.0)) + result.confidence) / count
            };
            performance_entry.accuracy = Some(performance_entry.avg_confidence.min(1.0));

            performance_entry
                .precision
                .entry("feature_dimensions".to_string())
                .and_modify(|value| *value += result.features.names.len() as f64)
                .or_insert(result.features.names.len() as f64);

            performance_entry
                .recall
                .entry("metadata_pairs".to_string())
                .and_modify(|value| *value += result.features.metadata.len() as f64)
                .or_insert(result.features.metadata.len() as f64);
        }

        // Update average processing time (blend current and historical signal)
        stats.avg_processing_time_ms = (stats.avg_processing_time_ms + per_result_avg) / 2.0;
    }
...
mod tests {
    ...
    fn test_structural_features() {
        ...
        let base64_idx = names
            .iter()
            .position(|n| n == "has_base64_pattern")
            .unwrap();
        ...
        let (names, values, _) = classifier.extract_structural_features(
