...
pub(crate) struct PatternStats {
    /// Number of built-in patterns loaded into the registry.
    pub(crate) patterns_loaded: usize,

    /// Number of custom patterns supplied at runtime.
    pub(crate) custom_patterns: usize,

    /// Total matches produced since initialization.
    pub(crate) total_matches: usize,

    /// Matches produced by the most recent scan.
    pub(crate) last_scan_matches: usize,
}
    ...
    || {
        ...
        patterns.insert(
            ...
            vec![(
                ...
                r#"(?s)\{[^}]*"type":\s*"service_account"[^}]*"private_key":\s*"[^"]+[^}]*\}"#
                    .to_string(),
                0.95,
            )],
        );
        ...
        patterns.insert(
            CredentialType::MongoDbConnectionString,
            vec![(
        ...
        patterns.insert(
            CredentialType::PostgreSqlPassword,
            vec![(
        ...
        patterns.insert(
            ...
            vec![
                ...
                (
                    ...
                    r"(sk_(?:test|live)_[0-9A-Za-z_\-]{16,})".to_string(),
                    0.95,
                ),
            ],
        );
        ...
        patterns.insert(
            CredentialType::Ed25519PrivateKey,
            vec![(
        ...
        patterns.insert(
            CredentialType::Pkcs12Bundle,
            vec![(
                "pkcs12_bundle".to_string(),
                r"(?i)[\w./\\-]+\.(?:p12|pfx)".to_string(),
                0.88,
            )],
        );
        ...
        patterns.insert(
            ...
            vec![
                (
                    ...
                    r"(?i)(password|pwd|pass)\s*[:=]\s*[\x22\x27]([^\x22\x27\s]{8,})[\x22\x27]"
                        .to_string(),
                    0.6,
                ),
                (
...
impl PatternRegistry {
    ...
    pub async fn new() -> Result<Self> {
        ...
        info!(
            "✅ Pattern registry initialized with {} pattern types",
    ...
    async fn load_builtin_patterns(&mut self) -> Result<()> {
        for (credential_type, pattern_definitions) in BUILTIN_PATTERNS.iter() {
            ...
            self.patterns
                .insert(credential_type.clone(), compiled_patterns);
        }

        if let Ok(mut stats) = self.stats.write() {
            stats.patterns_loaded = self.patterns.len();
        }
        Ok(())
    }
    ...
    pub fn add_custom_pattern(&mut self, pattern: CustomPattern) -> Result<()> {
        ...
        if let Ok(mut stats) = self.stats.write() {
            stats.custom_patterns = self.custom_patterns.len();
        }
    ...
    pub async fn scan_text(&self, text: &str) -> Result<Vec<PatternMatch>> {
        ...
        for (credential_type, patterns) in &self.patterns {
            for pattern in patterns {
                let pattern_matches = self
                    .scan_with_pattern(text, credential_type, pattern)
                    .await?;
        ...
        if let Ok(mut stats) = self.stats.write() {
            stats.last_scan_matches = matches.len();
            stats.total_matches = stats.total_matches.saturating_add(matches.len());
        }
    ...
    fn calculate_context_confidence(
        ...
        let mut applied_test_penalty = false;
        for indicator in &test_indicators {
            if context.contains(indicator) && !applied_test_penalty {
                confidence = (confidence - 0.15).max(0.0);
                applied_test_penalty = true;
            }
        }
    ...
    fn get_context_keywords(&self, credential_type: &CredentialType) -> Vec<String> {
        match credential_type {
            CredentialType::AwsAccessKey => vec![
                "aws".to_string(),
                "amazon".to_string(),
                "access".to_string(),
            ],
            CredentialType::GitHubToken => {
                vec!["github".to_string(), "git".to_string(), "token".to_string()]
            }
            CredentialType::DatabasePassword => vec![
                "database".to_string(),
                "db".to_string(),
    ...
    pub(crate) fn get_stats(&self) -> PatternStats {
        match self.stats.read() {
            Ok(stats) => stats.clone(),
            Err(_) => PatternStats::default(),
        }
    }
    ...
    async fn validate_with_external_service(
        ...
        for match_item in &mut matches {
                ...
                || match_item.value.to_lowercase().contains("example")
            {
                ...
                match_item
                    .metadata
                    .insert("validation_status".to_string(), "validated".to_string());
            }
        }

        debug!(
            "External validation completed for {} matches",
            matches.len()
        );
        Ok(matches)
    }
    ...
    async fn enhance_with_ml_scoring(
        &self,
        mut matches: Vec<PatternMatch>,
    ) -> Result<Vec<PatternMatch>> {
        ...
        for match_item in &mut matches {
            ...
            let has_entropy_indicators = match_item.value.chars().any(|c| c.is_ascii_uppercase())
            ...
            if has_entropy_indicators && context_length > 20 {
                ...
                match_item.metadata.insert(
                    "ml_enhancement".to_string(),
                    "high_entropy_detected".to_string(),
                );
            }
            ...
            match_item
                .metadata
                .insert("ml_processed".to_string(), "true".to_string());
        }
...
mod tests {
    ...
    async fn test_pattern_registry_creation() {
        ...
        assert!(stats.patterns_loaded >= registry.patterns.len());
    }
    ...
    async fn test_aws_access_key_detection() {
        ...
        for test_case in test_cases {
            ...
            let aws_matches: Vec<_> = matches
                ...
                .collect();
            assert!(
                !aws_matches.is_empty(),
                "No AWS access key detected in: {}",
                test_case
            );
        }
    }
    ...
    #[tokio::test]
    async fn test_password_env_var_detection() {
        let registry = PatternRegistry::new().await.unwrap();
        let test_text = "DB_PASSWORD=\"P@ssw0rd123!\"";

        let matches = registry.scan_text(&test_text).await.unwrap();
        assert!(
            matches
                .iter()
                .any(|m| matches!(m.credential_type, CredentialType::Password)),
            "Expected password detection in ENV assignment"
        );
    }

    #[tokio::test]
    async fn env_fixture_patterns_cover_expected_types() {
        let registry = PatternRegistry::new().await.unwrap();
        // Construct tokens dynamically to avoid storing secret-shaped literals in the repo.
        let slack_bot = format!(
            "xoxb-{}-{}-{}",
            "1234567890", "0987654321", "ABCDEFGHIJKLMNOPQRSTUVWX"
        );
        let slack_user = format!(
            "xoxp-{}-{}-{}-{}",
            "1234567890", "0987654321", "1122334455", "ABCDEFGHIJKLMNOPQRSTUVWXYZ123456"
        );
        let github = format!("ghp_{}", "1234567890abcdef1234567890abcdef1234");
        let stripe = format!("sk_test_{}", "abcdefghijklmnopqrstuvwxyz123456");
        let twilio = format!("AC{}", "0123456789abcdef0123456789abcdef");
        let aws_key = format!("AKIA{}", "IOSFODNN7EXAMPLE");
        let aws_secret = "wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY";
        let jwt = "eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIxMjMiLCJuYW1lIjoiVGVzdCBVc2VyIn0.signature";
        let cc = "4532015112830366";
...
"#
        ...
        let matches = registry.scan_text(&test_text).await.unwrap();
        let types: HashSet<_> = matches.iter().map(|m| m.credential_type.clone()).collect();
        ...
        assert!(
            ...
            "pattern registry missed expected types: {:?}",
    ...
    async fn test_custom_pattern() {
        ...
        assert!(matches!(
            custom_match.credential_type,
            CredentialType::Custom(_)
        ));
    }
    ...
    async fn test_enterprise_integration_hooks() {
        ...
        if enhanced_match.value.contains("test") {
            assert!(
                enhanced_match.confidence_score < 0.6,
