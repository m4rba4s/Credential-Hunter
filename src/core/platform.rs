...
impl Platform {
    ...
    pub async fn new(_config: &EchConfig) -> Result<Self> {
        ...
        info!(
            "   CPU: {} cores, SIMD: {}",
            info.cpu_info.logical_cores,
            if capabilities.simd_support.avx2 {
                "AVX2"
            } else if capabilities.simd_support.avx {
                "AVX"
            } else if capabilities.simd_support.sse {
                "SSE"
            } else if capabilities.simd_support.neon {
                "NEON"
            } else {
                "None"
            }
        ...
        debug!(
            io_buffer_size = optimizations.io_buffer_size,
            memory_allocator = %optimizations.memory_allocation,
            filesystem_opts = ?optimizations.filesystem_opts,
            network_opts = ?optimizations.network_opts,
    ...
    pub fn supports_feature(&self, feature: &str) -> bool {
        match feature {
            ...
            "simd" => {
                self.capabilities.simd_support.avx
    ...
    #[cfg(target_os = "linux")]
    #[allow(dead_code)]
    async fn linux_read_process_memory(
        &self,
        pid: u32,
        address: u64,
        size: usize,
    ...
    #[cfg(target_os = "windows")]
    async fn windows_read_process_memory(
        &self,
        pid: u32,
        address: u64,
        size: usize,
        ...
        Err(anyhow::anyhow!(
            "Windows memory reading not yet implemented"
        ))
    }

    #[cfg(target_os = "macos")]
    async fn macos_read_process_memory(
        &self,
        pid: u32,
        address: u64,
        size: usize,
...
mod tests {
    ...
    #[tokio::test]
    async fn test_platform_helper_invocations() {
        let config = EchConfig::default();
        let platform = Platform::new(&config).await.unwrap();

        // Best-effort calls; errors are acceptable on locked-down systems.
        let _ = platform.read_process_memory(std::process::id(), 0, 0).await;
        let _ = platform.scan_filesystem_optimized(".").await;
    }
}
